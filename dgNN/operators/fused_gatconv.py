...
class FusedGATFunction(torch.autograd.Function):
    ...
    def backward(ctx,grad_out):
        row_ptr,col_ind,col_ptr,row_ind,edge_max,edge_sum,edge_mask,in_feat,attn_row,attn_col=ctx.saved_tensors
        grad_out=grad_out.contiguous()
        # print('start backward')
        grad_feat,grad_attn_row,grad_attn_col=fused_gat.gat_backward(
            ctx.negative_slope,ctx.attn_drop,row_ptr,col_ind,col_ptr,row_ind,edge_max,edge_sum,edge_mask,in_feat,attn_row,attn_col,grad_out)
        # print('end backward')
